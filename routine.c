...
/*   Updated: 2025/12/04 16:32:14 by bedantas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philo_routine(void *arg)
{
	...
	while (p->data->flag_died != 1)
	{
		...
		{
			//espera? usleep(p->data->inp.time_to_die * 1000);
            print_action(&p, "died");
			p->data->flag_died == 1;
			break ;
		}
		...
		//comer: espera? usleep(p->data->inp.time_to_eat * 1000);
		print_action(&p, "is eating");
        
		pthread_mutex_unlock(p->left_fork);
		pthread_mutex_unlock(p->right_fork);
        
		//dormir: espera? usleep(p->data->inp.time_to_sleep * 1000);
